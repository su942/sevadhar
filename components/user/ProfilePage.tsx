...
const ProfilePage = () => {
  const [user, setUser] = useState(authStore.currentUser || { name: '', phone: '', email: '', photo: '' });
  ...
  useEffect(() => {
    // Subscribe to store changes to ensure UI stays in sync if status changes externally
  ...
  const handleBecomePartner = () => {
      // Direct upgrade without alert for smoother UX
  ...
  const handleSaveAddress = () => {
    if (tempAddress) {
        setAddresses(addresses.map(a => a.id === tempAddress.id ? tempAddress : a));
